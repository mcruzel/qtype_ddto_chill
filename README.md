...
Plugin Moodle autonome, dans son propre dépôt. Il n'hérite pas de
`qtype_ddwtos` du cœur (pas de points négatifs, stockage figé). Le dossier
d'installation **doit** s'appeler `ddto_chill`.
...
Le dossier **doit** s'appeler `ddto_chill` (pas `qtype_ddto_chill`, pas
`qtype_ddto_chill-main`). Sinon Moodle refuse le paquet à l'upload ZIP, ou
n'installe rien.

### Upload ZIP dans l'administration

1. Télécharger le ZIP GitHub.
2. Extraire, **renommer le dossier racine en `ddto_chill`**, rezipper ce dossier
   (le ZIP doit contenir `ddto_chill/version.php` à la racine du dossier, pas
   les fichiers à plat ni un dossier `qtype_ddto_chill-main`).
...
1. Copier ce dossier dans `question/type/` sous le nom `ddto_chill`
